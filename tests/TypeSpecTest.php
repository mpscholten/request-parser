...
class TypeSpecTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'id', '100');
        $this->assertInstanceOf(IntParser::class, $spec->int());
    }

    public function testFloat()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'ratio', '0.91');
        $this->assertInstanceOf(FloatParser::class, $spec->float());
    }

    public function testString()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'name', 'quintly');
        $this->assertInstanceOf(StringParser::class, $spec->string());
    }

    public function testUrl()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'referrer', 'https://www.quintly.com/');
        $this->assertInstanceOf(UrlParser::class, $spec->string()->url());
    }

    public function testEmail()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'emailAddress', 'john@doe.com');
        $this->assertInstanceOf(EmailParser::class, $spec->string()->email());
    }

    public function testOneOf()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'type', 'b');
        $this->assertInstanceOf(OneOfParser::class, $spec->oneOf(['a', 'b']));
    }

    public function testDateTime()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'createdAt', '01-01-2016');
        $this->assertInstanceOf(DateTimeParser::class, $spec->dateTime());
    }

    public function testJson()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'payload', '{}');
        $this->assertInstanceOf(JsonParser::class, $spec->json());
    }

    public function testYesNoBoolean()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'isAwesome', 'yes');
        $this->assertInstanceOf(YesNoBooleanParser::class, $spec->yesNoBoolean());
    }

    public function testBoolean()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'isAwesome', 'true');
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'hundreds', '100,200,300');
        $this->assertInstanceOf(CommaSeparatedIntParser::class, $spec->commaSeparated()->int());
    }

    public function testCommaSeparatedFloat()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'precipitation', '0.91,1.22,4.50');
        $this->assertInstanceOf(CommaSeparatedFloatParser::class, $spec->commaSeparated()->float());
    }

    public function testCommaSeparatedString()
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'tags', 'quintly,social,media,analytics');
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'eventsAt', '2016-01-01,2016-01-02');
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'payload', '{"a":5},{"a":6}');
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'answers', 'yes,no,yes');
    ...
    {
        $spec = new TypeParser($this->createExceptionFactory(), new MessageFactory(), 'answers', 'true,false,true');
